...
export class PatientListComponent implements OnInit, OnDestroy {
   ...
   * @param {InvoiceListService} _patientListService
   */
    constructor(private _patientListService: PatientListService, private _coreConfigService: CoreConfigService) {
  ...
  ngOnInit(): void {
    ...
    this._coreConfigService.config.pipe(takeUntil(this._unsubscribeAll)).subscribe(config => {
      ...
      if (config.layout.animation === 'zoomIn') {
        setTimeout(() => {
            this._patientListService.onInvoiceListChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.data = response;
            this.rows = this.data;
